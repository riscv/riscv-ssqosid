...
In a typical implementation the number of `RCID` bits (e.g., to support 10s of
`RCID`s) implemented may be smaller than the number of `MCID` bits (e.g., to 
support 100s of `MCID`s) implemented. It is a typical usage to associate a group
of applications/VMs with a common `RCID`. The group being associated with a
common `RCID` thus shares a common pool of resource allocations. The resource
allocation for the `RCID` is established to meet the SLA objectives of all
members of the group. If SLA objectives of one or more members of the group stop
being met, the resource usage of one or more members of the group may be
...
large pool of `MCID` speeds up this analysis. The usage motivates separate IDs
