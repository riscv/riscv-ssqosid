[[header]]
:description: RISC-V Quality-of-Service (QoS) Identifiers
:company: RISC-V.org
:revdate: 8/2022
:revnumber: 1.0
...
originator of the request. The QoS Identifiers extension provides a mechanism
to configure a hart with two identifiers - a resource control ID (`RCID`) and a
monitoring counter ID (`MCID`) - that accompany each request made by the hart
as the request transits through the network of resources.  `RCID` and `MCID`
have a global scope across all caches, interconnect, and memory controllers
that a request may access.
...
The QoS identifiers extension, introduces a read/write S/HS-mode register
- `sqoscfg` - to configure QoS Identifiers - `RCID` and `MCID` - to be used
with requests made by the hart when not in M-mode and a read/write M-mode 
register - `mqoscfg` - to configure QoS Identifiers - `RCID` and `MCID` - to be
used with requests made by the hart when in M-mode.
