...
Aaron Durbin, Allen Baum, Greg Favor, Ved Shanbhogue
...
mechanism to associate a hart with two identifiers - a resource control ID (`RCID`)
and a monitoring counter ID (`MCID`) - that accompany each request made by the hart
as the request transits through the network of resources. RCID and MCID have a 
global scope across all caches, interconnect, and memory controllers that a 
request may access.

Shared resource controllers (e.g., cache controllers, memory controllers, etc.), that
support QoS resource allocation capabilities, provide a mechanism to configure resource
usage limits (e.g., cache capacity, memory bandwidth, etc.) for each `RCID`. Shared 
resource controllers, that support QoS resource usage monitoring capabilities, implement
a per `MCID` counter to count events such as cache occupancy, memory bandwidth, etc.

The QoS identifiers extension, introduces a single read/write S/HS-mode register - `sqoscfg` - to 
associate QoS Identifiers - `RCID` and `MCID` - with software executing on a RISC-V hart.
